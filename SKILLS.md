...
18. For payments, `beforePayRecordedWith` **merges** both hooks' specs: the 721 hook's pay specification comes first (with `amount=0`, minting NFTs as a bonus), followed by the custom data hook's specs. The custom data hook controls the weight.

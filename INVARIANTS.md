...
- The contract reserves the project ID up front by calling `PROJECTS.createFor{value: msg.value}(address(this))` (`JBOmnichainDeployer.sol:_launchProjectFor`) — `address(this)` is the temporary holder of the NFT during the launch, never the configured `owner`. This prevents permissionless project creations from interleaving and invalidating subsequent hook deployments under that ID.
- **Fee-payer attribution.** Because the `createFor` owner argument is `address(this)`, the deployer implements `IJBPayerTracker` and sets `originalPayer = JBPayerTrackerLib.resolve(_msgSender())` immediately before the `createFor` call, clearing it back to `address(0)` immediately after. A `pay`-routing creation-fee receiver reads `originalPayer()` during the forwarded call and credits the resolved fee payer (the launching EOA, or that caller's upstream payer when the caller is itself an `IJBPayerTracker`) instead of this deployer. The slot is transient and never non-zero outside an in-flight launch.
...
- **`launchProjectFor(owner, projectUri, JBOmnichain721Config, JBRulesetConfig[], JBTerminalConfig[], memo, JBSuckerDeploymentConfig) payable → (projectId, hook, suckers)`** — `:204-227, 768-828`.
  ...
  - **Effect:** mints fresh `JBProjects` NFT to `address(this)` while advertising the resolved fee payer through `IJBPayerTracker.originalPayer` so the creation fee is attributed to the launching EOA, not this deployer; deploys a 721 tiers hook via `HOOK_DEPLOYER` with `salt = keccak256(abi.encode(_msgSender(), config.salt))`; wraps each ruleset's data-hook metadata to point at this contract and stores the underlying 721 + extra hook per ruleset; calls `CONTROLLER.launchRulesetsFor` (which configures terminals + accounting contexts + initial rulesets); transfers the 721 hook's `JBOwnable` ownership to the project; deploys suckers (if `suckerDeploymentConfiguration.salt != 0`) with salt mixed with `_msgSender()`; `safeTransferFrom`s the project NFT to `owner`.
